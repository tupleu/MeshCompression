...
impl Mesh {
	...
	pub fn edges(&mut self) -> &mut Vec<Edge> {
		&mut self.edges
	}
	...
	pub fn move_vertex(&mut self, from: VertexIndex, to: VertexIndex) {
		
	}

	pub fn nearest_vertex_to(&self, point: Point) -> VertexIndex {
		// search thru all triangles :(
		// expensive func
		// idc
	}

	pub fn collapse_edge(&mut self, index: EdgeIndex) {
		let new = self.get_or_add_vertex(self.midpoint(index).unwrap(), self.midcolor(index));
		self.move_vertex(self.start(index).unwrap(), new);
		self.move_vertex(self.end(index).unwrap(), new);
	}
	...
	pub fn end(&self, edge: EdgeIndex) -> Option<&Vertex> { 
		if edge.index as usize >= self.edges.len() { return None; }
		Some(&self.get_vertex(self.get_edge(edge).end()))
	}
	...
	pub fn midpoint(&self, index: EdgeIndex) -> Option<Point> {
		if index.index as usize >= self.edges.len() { return None; }
		
		let edge = self.get_edge(index);
		let start = self.get_vertex(edge.start()).pos();
		let end = self.get_vertex(edge.end()).pos();
		Some(Point::midpoint(start, end))
	}
	
	pub fn midcolor(&self, index: EdgeIndex) -> Option<[f32; 3]> {
		if index.index as usize >= self.edges.len() { return None; }
		
		let edge = self.get_edge(index);
		let start = self.get_vertex(edge.start()).color();
		let end = self.get_vertex(edge.end()).color();
		Some(average_color(start, end))
	}
